...
public class SwipeyTabsView extends RelativeLayout implements
	...
	public SwipeyTabsView(Context context, AttributeSet attrs, int defStyle) {
		...
		mOutsideOffset = (int) a.getDimension(
				R.styleable.ViewPagerExtensions_outsideOffset, -1);
	 ...
	 * 
	 * [1] -5 to -3 are outside the screen [2] -2 is outside the screen, may
	 * come into the screen when swiping right [3] -1 is inside the screen,
	 * aligned at the left [4] 0 is inside the screen, aligned at the center [5]
	 * +1 is inside the screen, aligned at the right [6] +2 is outside the
	 * screen, may come into the screen when swiping left [7] +3 to +5 are
	 * outside the screen
	 * 
	 * @param layout
	...
	private void calculateNewPositions(boolean layout) {
		...
		final int currentItem = mPosition + 1;

		for (int i = 0; i < mTabsCount; i++) {

			if (i < currentItem - 2)
				alignLeftOutside(i, false);
			else if (i == currentItem - 2)
				alignLeftOutside(i, true);
				...
				alignRightOutside(i, true);
			else if (i > currentItem + 2)
				alignRightOutside(i, false);
	...
	public void onPageScrolled(int position, float positionOffset,
		...
		for (int i = 0; i < mTabsCount; i++) {
			...
			if (y1 != y0)
				pos.currentPos = (int) (y0 + (y1 * x - y0 * x));
		}
